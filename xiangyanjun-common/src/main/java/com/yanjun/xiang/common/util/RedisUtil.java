...
import com.yanjun.xiang.common.lock.RedisDistributedLock;
...
public class RedisUtil<T> {
    ...
    public String getMemberValue(String key){
        RedisTemplate memberRedisTemplate = RedisDistributedLock.getMemberRedisTemplate();
        if(memberRedisTemplate==null){
            LOGGER.warn("RedisUtil memberRedisTemplate 为null");
            return null;
        }
        StringRedisTemplate memStringRedisTemplate = (StringRedisTemplate)memberRedisTemplate;
        return memStringRedisTemplate.opsForValue().get(key);
    }
